		...
		setTimeout(async () => {
			...
			try {
				...
				if (sessions.length > 0) {
					...
					for (const s of byProject.values()) {
						...
						await createTab(s.id, s.project || 'Shell', s.cwd || '%USERPROFILE%\\Desktop', pid, true, savedTab?.tabName || null);
						reconnected = true;
					}
				}
				...
				if (!reconnected && dbTabs.length > 0) {
					for (const dt of dbTabs) {
						...
						await createTab(dt.sessionId, dt.project || 'Shell', dt.cwd || '%USERPROFILE%\\Desktop', pid, false, dt.tabName || null);
						reconnected = true;
					}
				}
			} catch (e) {
			...
			if (!reconnected) {
				...
				for (const s of savedSessions) {
					...
					await createTab(s.sessionId, s.project || 'Shell', s.cwd || '%USERPROFILE%\\Desktop', s.projectId, false, s.tabName || null);
					reconnected = true;
				}
				saveSessionState();
			}
