...
app.post('/api/v1/clipboard-image', async (req, res) => {
  try {
    ...
    const dir = join(process.env.TEMP || '%USERPROFILE%\\AppData\\Local\\Temp', 'pan-clipboard');
    mkdirSync(dir, { recursive: true });
...
// GET /api/v1/needs?user=owner  → all needs with current decayed levels
...
app.use('/clipboard', express.static(join(process.env.TEMP || '%USERPROFILE%\\AppData\\Local\\Temp', 'pan-clipboard')));
...
app.post('/api/v1/voice/dictate', async (req, res) => {
  try {
    ...
    const stopFile = join('%USERPROFILE%\\AppData\\Local\\Temp', 'pan_dictate.wav.stop');
    if (_dictateActive) {
...
app.get('/api/v1/library', (req, res) => {
  try {
    ...
    walk(join(homedir(), '.claude', 'projects', 'C--Users-owner-Desktop-PAN', 'memory'), 'memory');
    // .pan project files
    try {
...
app.get('/api/v1/library/view', (req, res) => {
  try {
    ...
    const MEM = join(homedir(), '.claude', 'projects', 'C--Users-owner-Desktop-PAN').replace(/\\/g, '/');
