    ...
    tests: [
      {
        ...
        run: async (ctx) => {
          const port = getPort();
          const cwd = '%USERPROFILE%\\Desktop\\PAN';
    ...
    tests: [
      {
        ...
        run: async (ctx) => {
          ...
          const { ws, messages } = await connectWs({
            ...
            cwd: '%USERPROFILE%\\Desktop\\PAN', cols: 120, rows: 30
          });
          ctx.ws = ws; ctx.messages = messages;
    ...
    tests: [
      {
        ...
        run: async () => {
          ...
          const claudeMd = fs.readFileSync('%USERPROFILE%/Desktop/PAN/CLAUDE.md', 'utf8');
      ...
      },
      {
        ...
        run: async () => {
          ...
          const memoryDir = '%USERPROFILE%/.claude/projects/C--Users-owner-Desktop-PAN/memory';
      ...
      },
      {
        ...
        run: async () => {
          const fs = await import('fs');
          const briefing = '%USERPROFILE%/Desktop/PAN/.pan-briefing.md';
          const state = '%USERPROFILE%/Desktop/PAN/.pan-state.md';
          let source = 'none';
          let size = 0;
      ...
      },
      {
        ...
        run: async () => {
          const fs = await import('fs');
          const before = fs.readFileSync('%USERPROFILE%/Desktop/PAN/CLAUDE.md', 'utf8').length;
          await apiPost('/api/v1/inject-context', { cwd: '%USERPROFILE%\\Desktop\\PAN' });
          const after = fs.readFileSync('%USERPROFILE%/Desktop/PAN/CLAUDE.md', 'utf8');
