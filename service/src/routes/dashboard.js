...
router.get('/api/events', (req, res) => {
  ...
  if (projectPath) {
    ...
    const fwd = projectPath.replace(/\\/g, '/');                    // %USERPROFILE%/.../PAN
    const bk = fwd.replace(/\//g, '\\\\');                          // %USERPROFILE%\\...\\PAN (as in JSON)
...
router.get('/api/memory', (req, res) => {
  ...
  try {
    ...
    for (const projDir of projectDirs) {
      ...
      // Derive a readable project name from the dir name (C--Users-owner-OneDrive-Desktop-PAN → PAN)
