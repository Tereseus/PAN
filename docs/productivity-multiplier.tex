...
of PAN's voice-and-vision interaction paradigm over traditional
...
predicts that PAN-style interaction achieves a multiplicative speedup of
...
PAN eliminates most of these stages. The user speaks their intent in natural
...
\textbf{In PAN:} $T_{\text{navigate}}^{\text{PAN}} = 0$. There is no pointing.
...
\textbf{In PAN:} $T_{\text{decide}}^{\text{PAN}} \approx 0$. The user does not
...
\textbf{In PAN:} $M_{\text{occupied}} \approx 0\text{--}1$ (the user only needs
...
\subsection{PAN Paradigm}

\begin{equation}
  T_{\text{PAN}}(\tau) = T_{\text{intent}} + \underbrace{0}_{\text{no navigation}} + \underbrace{T_{\text{speak}}}_{\text{voice input}} + T_{\text{exec}} + T_{\text{verify}} + T_{\text{error}}^{\text{PAN}}
\end{equation}
...
error rate $\epsilon_{\text{STT}} \approx 0.05$~\cite{google2023}, but PAN's AI
...
The PAN Productivity Multiplier for task $\tau$ is:
\begin{equation}
  \boxed{\mathcal{M}(\tau) = \frac{T_{\text{trad}}(\tau)}{T_{\text{PAN}}(\tau)}}
  \label{eq:multiplier}
\end{equation}
\end{definition}
...
\end{equation}
\begin{equation}
  T_{\text{interaction}}^{\text{PAN}} = T_{\text{speak}} + T_{\text{error}}^{\text{PAN}}
\end{equation}

The \emph{interaction multiplier} is:

\begin{equation}
  \mathcal{M}_{\text{interact}} = \frac{T_{\text{interaction}}^{\text{trad}}}{T_{\text{interaction}}^{\text{PAN}}}
\end{equation}
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Phase} & \textbf{Traditional (ms)} & \textbf{PAN (ms)} \\
\midrule
...
Note: most of PAN's time is AI processing, not human effort. The human effort is
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Phase} & \textbf{Traditional (min)} & \textbf{PAN (min)} \\
\midrule
...
The PAN Productivity Multiplier $\mathcal{M}(\tau)$ grows superlinearly with
...
In PAN, the AI maintains all context internally. The human input is a single
utterance regardless of $\kappa$, plus verification:
\begin{equation}
  T_{\text{PAN}}(\kappa) = T_{\text{speak}} + T_{\text{AI}}(\kappa) + T_{\text{verify}}(\kappa)
\end{equation}
...
Therefore:
\begin{equation}
  \mathcal{M}(\kappa) = \frac{T_{\text{trad}}(\kappa)}{T_{\text{PAN}}(\kappa)} = \frac{\mathcal{O}(\kappa^{1+\gamma})}{\mathcal{O}(\kappa)} = \mathcal{O}(\kappa^{\gamma})
...
Combining all components, the complete PAN Productivity Multiplier is:
...
$C_{\text{eff}}^{\text{PAN}}$ & PAN cognitive capacity & 7 slots \\
\bottomrule
\end{tabular}
...
\begin{axis}[
    ...
    title={\textbf{PAN Productivity Multiplier vs.\ Task Complexity}},
    thick,
]

% Traditional / PAN ratio curve (superlinear)
\addplot[
    color=blue,
    very thick,
    domain=1:120,
...
The dashed red line shows the gain from voice input alone ($R_{\text{speak}} / R_{\text{type}} \approx 3.2\times$). The blue curve shows the \emph{total} PAN multiplier, which exceeds the speech-only gain because PAN also eliminates navigation, decision, context-switching, and memorization overhead---and these costs grow superlinearly with complexity.
...
PAN's interaction model has a second-order effect: it \textbf{democratizes
...
of PAN needs only to express intent in natural language and observe results.

To illustrate: one could configure a PAN voice command where the utterance
...
\begin{verbatim}
  User: "AHH"
  PAN:  [changes dashboard to dark mode]
...
We have shown that PAN's voice-first, context-aware interaction paradigm
